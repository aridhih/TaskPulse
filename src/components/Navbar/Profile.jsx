...
const Profile = ({ toggleProfile, toggleMenu }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-md z-50">
      <div className="relative bg-gradient-to-br from-blue-600 to-purple-700 p-6 rounded-2xl shadow-xl w-[400px] text-center text-white animate-fade-in">

        {/* Loading Overlay */}
        {profileLoader && (
          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center z-10 rounded-2xl">
        ...
        <button
          ...
          className="absolute top-3 right-4 text-lg text-white hover:text-gray-300 transition z-20"
        >
          ✖
        </button>
        ...
        <div className="relative w-32 h-32 mx-auto mb-4 z-20">
          {user?.photoURL ? (
            <img
              src={user.photoURL}
              alt="Profile"
              className="w-32 h-32 rounded-full object-cover"
              onLoad={() => setProfileLoader(false)}
              onError={() => setProfileLoader(false)} // in case image fails
            />
          ) : (
          ...
          {auth.currentUser?.uid && (
            <p className="text-gray-300">
              ...
              <MdOutlineContentCopy
                onClick={handleCopy}
                className="inline-block text-xl ml-2 cursor-pointer hover:text-gray-100 transition"
              />
